...
function renderPage(i){current=Math.max(0,Math.min(pages.length-1,i)); const p=pages[current]; $('#pageImg').src=p.img; $('#pageBadge').textContent=`الصفحة ${current+1} من ${pages.length}`; $('#pageTitle').textContent=p.title; $('#pageText').textContent=p.text; $('#progressBar').style.width=((current+1)/pages.length*100)+'%'; $('#prevBtn').disabled=current===0; $('#nextBtn').textContent=current===pages.length-1?'النهاية':'التالي'; earned=Math.max(earned,current+1); $('#stars').textContent='★ '.repeat(earned)+'☆ '.repeat(Math.max(0,5-earned)); setupGame(!!p.game)}
$('#prevBtn').onclick=()=>renderPage(current-1); $('#nextBtn').onclick=()=> current===pages.length-1?showScreen('home'):renderPage(current+1);
$('#readBtn').onclick=()=>{const p=pages[current]; if(currentAudio){currentAudio.pause(); currentAudio=null;} currentAudio=new Audio(`audio/page${current+1}.mp3`); currentAudio.play().catch(()=>{speechSynthesis.cancel(); const u=new SpeechSynthesisUtterance(p.text); u.lang='ar'; u.rate=.9; speechSynthesis.speak(u);});};
...
const colorImgs=['assets/coloring/coloring1.png','assets/coloring/coloring2.png','assets/coloring/coloring3.png','assets/coloring/coloring4.png','assets/coloring/coloring5.png'];
